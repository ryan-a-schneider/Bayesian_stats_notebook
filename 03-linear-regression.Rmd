...
Linear regression is not simply estimating parameters by pulling them from a known posterior distribution. Instead, it uses a relationship between the parameters, specified below, to predict values of $\mu$.

In classic linear regression, the mean of a normally distributed variable is assumed to be a linear function of the parameters $\alpha$ and $\beta$; i.e., each mean, $\mu_i$, is a function of $\alpha$ (the intercept, a constant) and $\beta$ (the slope; i.e., the rate at which the observed-variable X changes the mean).

The formula for the linear model is thus:
...
When building a linear model, you have to specify each component in the above formula. There are three parameters that need priors:

-   $\alpha$, which is th intercept

-   $\beta$, which is the slope that describes the rate of change for the included variable/effect

-   and $\sigma$, which is necessary to include to describe the variation around the individual means being predicted.
