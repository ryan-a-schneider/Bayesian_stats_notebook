...
This is a test of the assumption that the variances of the independent groups' distributions on a continuous variable are **similar, equal, or roughly equal.** If the variances in the groups are different their distributions are different, which makes them incomparable. If you violate this assumption you need to do something to fix it before moving forward. Usually you'd use Lavene's Test

> Strictly speaking, the regression model assumes that each residual is generated from a normal distribution with mean 0, and (more importantly for the current purposes) with a standard deviation σ that is the same for every single residual. In practice, it's impossible to test the assumption that every residual is identically distributed. Instead, what we care about is that the standard deviation of the residual is the same for all values of Yˆ , and (if we're being especially paranoid) all values of every predictor X in the model (R for psychology)

```{r}
plot(model_1, 3) # horizontal line with equally spread points is a good indication

performance::check_homogeneity(model_2)
```

#### Linearity of Residuals

> A pretty fundamental assumption of the linear regression model is that relationship between X and Y actually be linear! Regardless of whether it's a simple regression or a multiple regression, we assume that the relatiships involved are linear. (R for Psychology, p. 474)
...
In multiple regression, the assumption of normality applies only to the residuals, *not* the independent variables. "Simply put, the residuals are the error in the relationship between the independent variables and the dependent variable in a regression model. These should be normally distributed.
...
Visual graphs of residuals show if the variables are following a path that would be expected, if your data was normally distributed. Only looking for visual evidence of "extreme" problems.

You can use two different types of graphs to check this:
...
##### ***Significance test checks***

The *Kolmogorov-Smirnov Test* or the *Shapiro-Walk test* both work here. Both basically compare data with a normal distribution that has the same M and SD. A p \< .05 means that distribution of variable is significantly different from normal. This will likely be significant with large sample, so it should be combined with a visual check!!!
...
```{r}
#### Visual checks ####
qplot(model_1$residuals) #residuals histogram. Should have a normal distribution
...
##### ***Fixing non-normal data (normal linear models).*** Consider the following from the Kline textbook:

> "Some variables are expected to have non-normal distributions, such as reports of alcohol or drug use and certain personality characteristics. If so, then transofmring an inherently non-normal variable to force a normal distribution may fundamentally alter it. In this case, it would be better to use a different estimation method for continuous outcomes in SEM, one that does not assume normality, such as robust maximum likelihood." (p. 77)

> "Normalizing transformations may be more useful when there is no expectation of normlaity, or metrics of outcome variables are arbitrarty." (p. 77)

In other words, consider the information contained on the scale and how it would be changed/what would be lost before you transform it.

#### Heteroscadasticity: Non-constant Error Variance

AKA "heteroscedasticity", or inequality of error variances.

> The linear regression algorithm assumes that there is a linear relationship between the parameters of independent variables and the dependent variable Y. If the true relationship is not linear, we cannot use the model as the accuracy will be significantly reduced. ([link](https://towardsdatascience.com/linear-regression-assumptions-why-is-it-important-af28438a44a1))

"Residual" is the difference between the predicted score and actual score. The amount of miss-prediction, in other words, that your model is making. You want this error rate *to be constant along the values of the DV*. If the error rate is not constant, it means the model is predicting better in some areas than in others, which is problematic.It's really problematic if the model only works for some people or in some ranges and not others.

Two ways to do this:

-   Check the normality of the residuals (*these should be normally distributed*)

-   Check the plot of the (standardized) residuals against predicted scores (*should be evenly distributed and scattered*)

If you have problems with this you can add or remove some variables, transform the data, or switch to a weighted least squares model.

```{r}
#### Visual check ####
plot(model_2, 1) # want to see a horizontal line, without distinct patterns

#### Significance test ####
performance::check_heteroscedasticity(model_2) #You want to see a non-significant result; significance implies assumption violation
...
*Methods to detect collinearity among three or more continuous variables:* You have three different options to do this...
...
Example write up: Tests to check the assumption of collinearity indicated that correlation between the predictors was not an issue (Sex, *VIF*= 1.00; Child, *VIF*=1.00; Mean Negative Affect, *VIF*=1.01).
