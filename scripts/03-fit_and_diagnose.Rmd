...
-   In logistic models:
    ...
    -   The priors are on the **log-odds** scale; you're specifying your prior belief in terms of the log-odds of y=1.

    -   The intercept value represents the log-odds of success (y=1), when the predictors are held constant at 0.

    -   It's easiest (for me) to think it terms of odds ratios, and then take the inverse of that with `log()` to find the coefficient that comes out to, when setting a prior.
