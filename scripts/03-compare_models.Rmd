...
>
> If elpd difference is larger than 4, then compare that difference to standard error of `elpd_diff`...When the difference (`elpd_diff`) is larger than 4, the number of observations is larger than 100, and the model is not badly misspecified, then normal approximation and SE are quite reliable description of the uncertainty in the difference.
...
>
> See more in "Interpreting p_loo when Pareto-k\^ is large" in LOO Glossary.
...
Since the estimated difference between the model's ELPD is small, the models can be said to have comparable predictive performance.

However, if we compare 5.1 to b5.2...
...
```{r}
loo_estimates |> mutate(diff_check=ifelse(elpd_diff>(abs(2*se_diff)), "Big", "Not big"))
```
