...
If you want to fit multiple models to test and see which is best, rather than make a huge code block with copy-pasted code of a slew of models, better to make use of the `update()` function, which updates an original model by refitting it with the specified changes.

The example below refits and creates a second version of the logistic model above by removing one of the predictors.

```{r}
logistic_fit2=update(fit2, formula. =  ~ . - height_z)
```

## BRMS SETTINGS

Notice the following line near the end of the model: `iter = 2500, warmup = 500, chains = 4, cores = 4, seed = 21`. This section describes what to do with these settings.

### iter

`iter` is the number of steps taken in each chain.
...
The two below are probably what you'll want to use. They give you info on *only the main effects*. No sigma, and no lp\_\_.
