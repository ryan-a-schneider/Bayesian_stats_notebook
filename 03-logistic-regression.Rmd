...
-   `iter` sets the number of iterations ("steps") per chain...

## Example models from Kruschke's DBDA
...
fit2_test=update(fit2, formula. =  ~ . - height_z)
```

-   *Still need to know more about* `iter`, `warmup`, `chains`, and `seed`.

-   Also, can you do a student-t prior in brms?
...
```{r}
 pts2_Bayes_ogm=stan_glm(accept ~ grp_all, 
                    family= binomial(link = "logit"), 
                    data= data_by_condition$pts_2, 
                    prior = priors,
                    ...
                    #QR = FALSE, 
                    #sparse = FALSE,
                    chains=4,iter=60000,cores=4, # 5-6k steps fine if not using Bayes factors; 50-60k if using them.
                    diagnostic_file=file.path(tempdir(), "df2.csv")) # can delete this if not using Bayes factors
```

## Details

-   `mean_PPD` is for sampling from the Prior Predictive Distribution...
